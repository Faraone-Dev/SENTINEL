...
contract SentinelRegistry {
        ...
        external
    {
        ...
        for (uint256 i = 0; i < erc721Revokes.length;) {
            ...
            emit NFTApprovalRevoked(
                msg.sender, erc721Revokes[i].collection, erc721Revokes[i].tokenId
            );
            unchecked {
                ++i;
                ++totalRevoked;
        ...
        for (uint256 i = 0; i < operatorRevokes.length;) {
            ...
            emit OperatorRevoked(
                msg.sender, operatorRevokes[i].collection, operatorRevokes[i].operator
            );
            unchecked {
                ++i;
                ++totalRevoked;
