...
contract SentinelRegistryFuzzTest is Test {
    ...
    function setUp() public {
        ...
        for (uint256 i = 0; i < NUM_SPENDERS; i++) {
    ...
    function testFuzz_BatchRevokeERC20AnySize(uint256 batchSize) public {
        ...
        for (uint256 i = 0; i < batchSize; i++) {
            uint256 tokenIdx = i % NUM_TOKENS;
            uint256 spenderIdx = i % NUM_SPENDERS;

            // Set allowance
            tokens[tokenIdx].approve(spenders[spenderIdx], 1000e18);

            revokes[i] = SentinelRegistry.ERC20Revoke({
                token: address(tokens[tokenIdx]), spender: spenders[spenderIdx]
            });
        }
        ...
        // Verify all revoked
        for (uint256 i = 0; i < batchSize; i++) {
            assertEq(tokens[i % NUM_TOKENS].allowance(address(this), spenders[i % NUM_SPENDERS]), 0);
        }
    }
    ...
    function testFuzz_BatchRevokeERC721RandomIds(uint256 id1, uint256 id2, uint256 id3) public {
        // Ensure unique IDs
        ...
        // Batch revoke
        SentinelRegistry.ERC721Revoke[] memory revokes = new SentinelRegistry.ERC721Revoke[](3);
        ...
        public
    {
        ...
        for (uint256 i = 0; i < erc20Count; i++) {
            ...
            erc20Revokes[i] = SentinelRegistry.ERC20Revoke({
                token: address(tokens[i % NUM_TOKENS]), spender: spenders[i % NUM_SPENDERS]
            });
        }
        ...
        for (uint256 i = 0; i < erc721Count; i++) {
            ...
            erc721Revokes[i] = SentinelRegistry.ERC721Revoke({
                collection: address(nfts[i % NUM_TOKENS]), tokenId: i + 1000
            });
        }
        ...
        for (uint256 i = 0; i < operatorCount; i++) {
            ...
            opRevokes[i] = SentinelRegistry.OperatorRevoke({
                collection: address(nfts[i % NUM_TOKENS]), operator: spenders[i % NUM_SPENDERS]
            });
        }
    ...
    function testFuzz_RevokeZeroAllowanceSucceeds(uint256 seed) public {
        uint256 tokenIdx = seed % NUM_TOKENS;
        uint256 spenderIdx = seed % NUM_SPENDERS;

        // Never approved - allowance already 0
        assertEq(tokens[tokenIdx].allowance(address(this), spenders[spenderIdx]), 0);

        // Should still succeed
        vm.prank(address(this), address(this));
        registry.revokeERC20(address(tokens[tokenIdx]), spenders[spenderIdx]);
    ...
    function testFuzz_GasScalesLinearly(uint256 batchSize) public {
        ...
        assertLt(gasPerItem, 50_000, "Gas per item too high");
    }
}
