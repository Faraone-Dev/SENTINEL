...
contract SentinelSecurityTest is Test {
    ...
    function test_Security_LargeBatchGasLimit() public {
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            revokes[i] = SentinelRegistry.ERC20Revoke({
                token: address(token), spender: address(uint160(i + 1))
            });
        }
    ...
    function test_Security_BatchIndexBounds() public {
        ...
        SentinelRegistry.ERC20Revoke[] memory revokes = new SentinelRegistry.ERC20Revoke[](100);

        for (uint256 i = 0; i < 100; i++) {
            address sp = address(uint160(i + 1));
            vm.prank(address(this), address(this));
            token.approve(sp, 1e18);
            revokes[i] = SentinelRegistry.ERC20Revoke({ token: address(token), spender: sp });
        }
    ...
    function test_Security_BatchPartialFailure() public {
        ...
        SentinelRegistry.ERC20Revoke[] memory revokes = new SentinelRegistry.ERC20Revoke[](2);
        revokes[0] = SentinelRegistry.ERC20Revoke({ token: address(good), spender: spender });
        revokes[1] = SentinelRegistry.ERC20Revoke({ token: address(bad), spender: spender });
