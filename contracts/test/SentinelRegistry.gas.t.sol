...
contract NaiveRevoker {
    function revokeERC20(address token, address spender) external {
        ...
        (bool success,) =
            token.call(abi.encodeWithSignature("approve(address,uint256)", spender, 0));
    ...
    function batchRevokeERC20(address[] calldata tokens, address[] calldata spenders) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            (bool success,) =
                tokens[i].call(abi.encodeWithSignature("approve(address,uint256)", spenders[i], 0));
            require(success, "Revoke failed");
        }
    }
}
...
contract SentinelGasBenchmarkTest is Test {
    ...
    function _benchmarkBatch(uint256 size) internal {
        ...
        SentinelRegistry.ERC20Revoke[] memory revokes = new SentinelRegistry.ERC20Revoke[](size);
        for (uint256 i = 0; i < size; i++) {
        ...
        uint256 savingsPercent =
            naiveGas > 0 ? (uint256(savings > 0 ? savings : -savings) * 100) / naiveGas : 0;
    ...
    function test_Benchmark_BatchRevokeERC721() public {
        ...
        // Prepare batch
        SentinelRegistry.ERC721Revoke[] memory revokes = new SentinelRegistry.ERC721Revoke[](size);
        for (uint256 i = 0; i < size; i++) {
            revokes[i] = SentinelRegistry.ERC721Revoke({
                collection: address(nfts[i % NUM_TOKENS]), tokenId: i + 100
            });
        }
    ...
    function test_Benchmark_MixedBatchRevoke() public {
        ...
        // Prepare batches
        SentinelRegistry.ERC20Revoke[] memory erc20Revokes = new SentinelRegistry.ERC20Revoke[](5);
        for (uint256 i = 0; i < 5; i++) {
            erc20Revokes[i] = SentinelRegistry.ERC20Revoke({
                token: address(tokens[i]), spender: spenders[i % NUM_SPENDERS]
            });
        }
        ...
        for (uint256 i = 0; i < 3; i++) {
            erc721Revokes[i] =
                SentinelRegistry.ERC721Revoke({ collection: address(nfts[i]), tokenId: i + 200 });
        }
        ...
        for (uint256 i = 0; i < 2; i++) {
            opRevokes[i] = SentinelRegistry.OperatorRevoke({
                collection: address(nfts[i]), operator: spenders[i]
            });
        }
